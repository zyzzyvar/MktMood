...
- **方向状态**：原有 `marketScore`，判断市场更偏进攻、防守还是中性拉扯。
- **风险状态**：新增 `riskRegime`，判断当前是不是平静、波动升温、高波动风险、机械去杠杆压力，或高波动修复期。

这能避免“方向分数多空抵消，所以显示中性”被误解为“市场很平静”。例如 `中性拉扯 · 高波动修复期` 表示方向上多空抵消，但波动、相关性、异动密度或杠杆脆弱度仍值得重点关注。
